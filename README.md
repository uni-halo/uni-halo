<p align="center">
    <a href="https://uni-halo.ialley.cn" target="_blank" rel="noopener noreferrer">
        <img width="100" src="https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/logo.png" alt="uni-halo logo" />
    </a>
</p>

<p align="center"><b>【UniHalo v3.x】</b> 支持 Halo2.x (不支持专业/商城版完整功能) 移动端多端项目，值得一试。</p>

<br />
<p align="center">
	<a href="https://www.xiaoxiaomo.cn">作者主页</a>
	<a href="https://blog.xiaoxiaomo.cn">作者博客</a>
	<a href="https://uni-halo.ialley.cn">官网主页</a>
	<a href="https://uni-halo-doc.ialley.cn">官方文档</a>
	<a href="https://github.com/uni-halo/uni-halo">仓库地址</a>
	<a href="https://github.com/uni-halo/uni-halo-plugin">插件仓库</a>
</p>

---

如果您觉得这个项目对您有帮助，可以请作者喝杯饮料鼓励一下，同时为了项目能够持续发展，可以根据您的喜好支持一下本项目哦，非常感谢您的支持，作者也会更有动力持续维护和更新新的功能哦~

|                                                 支付宝                                                 |                                                微信                                                 |                                                QQ                                                 |
|:---------------------------------------------------------------------------------------------------:|:-------------------------------------------------------------------------------------------------:|:-------------------------------------------------------------------------------------------------:|
| ![支付宝赞助](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/author/rewards/ZFBRewardCode.png) | ![微信赞助](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/author/rewards/WXRewardCode.png) | ![QQ赞助](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/author/rewards/QQRewardCode.png) |

## 📖 简介

**uni-halo** 是一款基于 [Halo 2.x](https://www.halo.run/) 开放 API 打造的免费开源博客多端应用。一套源码，编译为 **微信小程序（推荐）/ APP / H5**，配合 [Uni Halo 配置插件](https://github.com/uni-halo/uni-halo-plugin) 实现「改内容不改代码」——文章、公告、轮播、恋爱日记等全部内容均可在 Halo 后台动态配置。

## ✨ 特性一览

| | 特性 | 说明 |
|:---:|---|---|
| 📱 | **多端编译** | 基于 uni-app，一套源码编译微信小程序、APP、H5 |
| 🔌 | **插件化配置** | 页面内容、功能开关全部由配套插件管理，无需改源码 |
| 💕 | **恋爱日记** | 相册（密码锁定，页内解锁）/ 清单 / 故事，专为秀恩爱设计 |
| 🔐 | **账号体系** | 账号密码登录、注册、微信一键登录与绑定，按角色控制可见内容 |
| 📲 | **移动端管理** | 管理员可在手机上直接管理恋爱相册、清单、故事与瞬间 |
| 🎨 | **现代技术栈** | Vue3 + TypeScript + Vite + UnoCSS + wot-ui，界面精美、开发体验佳 |
| 🆓 | **完全开源** | 应用与插件源码全部开放，Apache License 2.0 |

## 🧩 功能模块

### 基础功能

文章列表 / 分类 / 标签 / 详情 / 归档 / 搜索 / 公告 / 图库与我的收藏 / 瞬间 / 友情链接（支持投稿申请）/ 投票中心 / 数据看板 / 联系博主 / 偏好设置

### 💕 恋爱日记（特色）

精心设计的秀恩爱模块：情侣甜蜜相册（支持查看密码，页内解锁，服务端保证安全）、恋爱记录清单、我们的故事。配套插件内置恋爱日记前台模板，站点主题装上插件即可直接展示。

![恋爱日记](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/恋爱日记.jpg)

### 🔐 登录与管理

- 账号密码登录、注册、微信小程序一键登录与微信绑定
- 登录后使用 Halo 原生令牌，内置登录限流保护
- 按角色控制菜单与按钮的可见性，管理员与普通用户看到不同的功能
- 管理员移动端直管：恋爱相册 / 恋爱清单 / 恋爱故事 / 瞬间

### 其他

站点维护模式页 / 公告通知 / 应用版本检查更新 / 数据统计可视化

## 📸 应用截图

### 主界面（TabBar）

| 首页 | 分类 | 图库 | 瞬间 | 博主 |
|:---:|:---:|:---:|:---:|:---:|
| ![首页](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/首页.png) | ![分类](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/分类.png) | ![图库](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/图库.png) | ![瞬间](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/瞬间.png) | ![博主](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/博主.png) |

### 💕 恋爱日记（特色功能）

| 恋爱主页 | 恋爱相册 | 恋爱清单 | 我们的故事 |
|:---:|:---:|:---:|:---:|
| ![恋爱日记](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/恋爱日记.png) | ![恋爱相册](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/恋爱相册.png) | ![恋爱清单](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/恋爱清单.png) | ![恋爱故事](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/恋爱故事.png) |

### 📲 移动端管理（管理员）

| 相册管理 | 瞬间管理 |
|:---:|:---:|
| ![相册管理](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/相册管理.png) | ![瞬间管理](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/app/v3.x/瞬间管理.png) |

### 🖼️ 恋爱日记前台模板（主题接入）

| 恋爱日记主页 | 恋爱相册 | 恋爱清单 | 我们的故事 |
|:---:|:---:|:---:|:---:|
| ![恋爱日记主页](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/plugin/v3.x/前台模板.png) | ![恋爱相册](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/plugin/v3.x/前台模板-恋爱相册.png) | ![恋爱清单](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/plugin/v3.x/前台模板-恋爱清单.png) | ![我们的故事](https://gcore.jsdelivr.net/gh/uni-halo/uni-halo-static/screenshots/plugin/v3.x/前台模板-恋爱故事.png) |

> 更多页面截图与功能演示，请访问 [官网](https://uni-halo.ialley.cn) 或 [官方文档](https://uni-halo-doc.ialley.cn)。

## 🚀 快速开始

### 环境要求

- [Halo](https://www.halo.run/) ≥ 2.26（站点程序）
- Node.js ≥ 20、pnpm ≥ 9
- 发布微信小程序需 [微信开发者工具](https://developers.weixin.qq.com/miniprogram/dev/devtools/download.html)

### 本地运行

```bash
# 1. 克隆源码
git clone https://github.com/uni-halo/uni-halo.git

# 2. 安装依赖
cd uni-halo
pnpm install

# 3. 启动开发服务（H5 预览）
pnpm dev:h5

# 4. 编译微信小程序（用微信开发者工具导入 dist/dev/mp-weixin）
pnpm dev:mp-weixin
```

> ⚠️ 运行前请先配置站点地址等环境变量，并安装启用 [Uni Halo 配置插件](https://github.com/uni-halo/uni-halo-plugin)。完整的部署、配置与发布流程请阅读 [部署指南](https://uni-halo-doc.ialley.cn/deploy/intro)。

## 🔗 配套插件

[Uni Halo 配置插件](https://github.com/uni-halo/uni-halo-plugin) 是本应用的核心配套插件，提供：

- **内容管理**：横幅轮播、公告、友链（投稿审核 + 邮件通知）、恋爱相册 / 清单 / 故事
- **应用管理**：应用信息维护、版本管理与检查更新
- **恋爱日记前台模板**：任何主题零成本接入恋爱日记页面
- **安全与登录**：验证码防护、移动端登录开关与微信密钥配置
- **主题扩展**：小程序悬浮窗、平台接入（第三方插件）

下载安装：[插件 Releases](https://github.com/uni-halo/uni-halo-plugin/releases) ｜ [Halo 应用市场](https://www.halo.run/store/apps/app-aukgwe3y)

## 📚 相关资源

| 资源 | 地址 |
|---|---|
| 作者主页 | https://www.xiaoxiaomo.cn |
| 作者博客 | https://blog.xiaoxiaomo.cn |
| 官网主页 | https://uni-halo.ialley.cn |
| 官方文档 | https://uni-halo-doc.ialley.cn |
| 应用仓库 | https://github.com/uni-halo/uni-halo |
| 插件仓库 | https://github.com/uni-halo/uni-halo-plugin |
| Halo 应用市场 | https://www.halo.run/store/apps/app-aukgwe3y |

## 💬 使用交流

- 提交 Issue：[GitHub Issues](https://github.com/uni-halo/uni-halo/issues)
- QQ 交流群：**632969367**
- 微信交流群：微信交流群时效性较短，请先加入 QQ 群，再被邀请进微信群

## 🤝 参与贡献

uni-halo 还在持续更新中，欢迎您的参与！无论是提交建议、反馈问题还是提交 PR，我们都非常乐意接受。

欢迎通过 [GitHub Issues](https://github.com/uni-halo/uni-halo/issues) 反馈问题，或提交 Pull Request 共同完善项目。感谢所有给 uni-halo 贡献过代码的[开发者](https://github.com/uni-halo/uni-halo/graphs/contributors)！

<a href="https://github.com/uni-halo/uni-halo/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=uni-halo/uni-halo" />
</a>

## 📄 开源许可

本项目基于 [Apache License 2.0](./LICENSE) 协议开源，本着一切免费的原则，原则上不会收取任何费用及版权，请遵守开源协议，放心使用。

## 🙏 致谢

uni-halo 得益于以下优秀项目的支撑，在此特别感谢这些优秀的项目！

- [Halo](https://halo.run/)：一款好用又强大的开源建站工具
- [uni-app](https://uniapp.dcloud.net.cn/)：使用 Vue.js 开发所有前端应用的框架，一套代码多端编译
- [unibest](https://unibest.tech/)：基于 uni-app + Vue3 + Vite + TypeScript 的最佳实践跨端开发框架
- [wot-ui](https://wot-ui.cn/)：高颜值、轻量化的 uni-app 组件库，全面支持 Vue3 多端
- [mp-html](https://jin-yufeng.gitee.io/mp-html/#/)：强大的小程序富文本组件
