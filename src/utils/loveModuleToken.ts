 ...
 * 模块 scope：ourStory(恋爱故事) / lovePhoto(恋爱相册) / loveDaily(恋爱清单)
 */
...
export type LoveModuleKey = 'ourStory' | 'lovePhoto' | 'loveDaily'

/** 读取指定模块的解锁 token（未解锁则 undefined） */
...
/** 保存指定模块的解锁 token（插件端 30 分钟有效，过期由服务端 401 兜底重新解锁） */
