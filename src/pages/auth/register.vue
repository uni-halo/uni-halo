...
async function doRegister() {
  ...
  // 邮箱必填且校验格式(发送验证码/提交均校验)
  const trimmedEmail = email.value.trim()
  if (!isValidEmail(trimmedEmail)) {
    uni.showToast({ icon: 'none', title: '请填写正确的邮箱' })
    return
  }
  // 站点开启注册邮箱验证时,验证码必填(服务端 Halo signUp 也会 fail closed 校验)
  if (emailVerifyRequired.value && !emailCode.value.trim()) {
  ...
  try {
    await tokenStore.register({
      ...
      // 邮箱必填,直传用户输入
      email: trimmedEmail,
      ...(emailVerifyRequired.value ? { emailCode: emailCode.value.trim() } : {}),
    })
...
<template>
  <view class="relative box-border min-h-screen w-screen flex flex-col overflow-hidden bg-[#f5fae8]">
      ...
      <view class="box-border w-full flex flex-1 flex-col items-center justify-center gap-y-4 pt-26">
        ...
        <view class="box-border w-full flex flex-col items-center">
          <view class="uh-global-card-glass uh-shadow-xs box-border w-full rounded-2xl p-4">
            <wd-input
              v-model="username" custom-class="uh-register-input" prefix-icon="user" no-border
              placeholder="请输入账号" :disabled="loading"
            />
            <wd-input
              v-model="displayName" custom-class="uh-register-input mt-3" prefix-icon="user"
              no-border placeholder="请输入昵称(选填)" :disabled="loading"
            />
            <wd-input
              v-model="password" custom-class="uh-register-input mt-3" prefix-icon="lock"
              show-password no-border placeholder="请输入密码" :disabled="loading"
            />
            <wd-input
              v-model="confirmPassword" custom-class="uh-register-input mt-3" prefix-icon="lock"
              show-password no-border placeholder="请再次输入密码" :disabled="loading" @confirm="doRegister"
            />

            <wd-input
              v-model="email" custom-class="uh-register-input mt-3" prefix-icon="email"
              no-border :placeholder="emailPlaceholder" :disabled="loading"
            />

            <template v-if="emailVerifyRequired">
              <view class="mt-3 flex items-center gap-x-2">
                <wd-input
                  v-model="emailCode" custom-class="uh-register-input flex-1"
            ...
            <view class="mt-4 flex items-start justify-center gap-x-1.5 px-2" @click="agreed = !agreed">
              <view
                class="check-circle mt-2rpx shrink-0 text-gray-900"
    ...
    <uh-agreement-popup
      v-model="agreementPopupVisible" :contents="agreementContents" :initial-tab="popupType"
