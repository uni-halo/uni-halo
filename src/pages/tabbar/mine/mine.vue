	...
	<view class="box-border min-h-screen w-screen bg-page pb-2">
		...
		<view class="relative h-96 w-full bg-cover bg-no-repeat" :style="[calcProfileStyle]">
			<view class="relative z-6 h-full flex flex-col items-center justify-center">
				<image class="uh-global-card-glass border-3 h-22 w-22 rounded-full" :src="bloggerInfo.avatar"
					mode="aspectFill" />
			...
			<!-- 过渡 -->
			<view class="absolute bottom-0 left-0 z-100 h-18 w-full bg-gradient-to-b from-black/0 to-page" />
		</view>

		<!-- 站点统计 -->
		<view v-if="featureMode==='list'"
		...
		<template v-if="featureMode==='base'">
			<view v-if="commonFeatures.length!==0" class="relative z-100 -mt-20 box-border overflow-hidden p-4">
				<view class="box-border uh-global-card-glass uh-shadow-xs border p-3 grid grid-cols-4 gap-2 rounded-lb-2xl rounded-rb-2xl rounded-lt-3xl rounded-rt-3xl">
					<view v-for="(nav) in commonFeatures" :key="nav.key"
						class="uh-global-card-glass uh-shadow-xs rounded-2xl flex flex-col items-center justify-between p-2"
			...
			<template v-if="otherFeatures.length!==0">
				<view class="box-border px-4 mt-2 mb-3">
					<uh-section-title>
						其他功能
					</uh-section-title>
				</view>
				<view class="uh-global-card-glass uh-shadow-xs mx-4 overflow-hidden rounded-2xl">
					<view v-for="(nav, index) in otherFeatures" :key="nav.key"
