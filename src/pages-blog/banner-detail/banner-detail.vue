  ...
  <view class="box-border min-h-screen w-screen bg-page pb-safe">
    ...
    <view v-else-if="detail" class="box-border p-4">
      ...
      <view v-if="hasLink" class="fixed bottom-0 left-0 right-0 box-border px-4 pb-safe">
        <!-- #ifndef APP-PLUS -->
        <uh-button class="flex-1" custom-class="uh-global-card-glass border w-full !rounded-full py-2.5 font-medium !text-3xs !bg-primary" @click="handleCopyLink">
          复制原文地址
        </uh-button>
        <!-- #endif -->
        <!-- #ifdef APP-PLUS -->
        <uh-button class="flex-1" custom-class="uh-global-card-glass border w-full !rounded-full py-2.5 font-medium !text-3xs !bg-primary" @click="handleOpenLink">
          访问链接
        </uh-button>
        <!-- #endif -->
      </view>
