<script lang="ts" setup>
	...
	async function handleGetData() {
		...
		try {
			...
			const res = await getPostByTagName(name.value, reqParams as IPostListReq)
	...
	onShareAppMessage(() => ({
		...
		path: `/pages-blog/tag-articles/tag-articles?name=${name.value}&title=${pageTitle.value}`,
	}))
...
<template>
	<view class="app-page min-h-screen w-screen flex flex-col bg-[#fafafd]">
		...
		<wd-sticky :offset-top="offsetTop">
			<view class="w-screen box-border flex items-center gap-2 px-3 py-2">
				<view v-for="opt in sortOptions" :key="opt.key" class="box-border uh-global-card-glass uh-shadow-xs border rounded-full px-3 py-1 text-xs"
		...
		<view v-if="loadingStatus !== 'success'">
			<uh-data-loading :loading-status="loadingStatus" empty-text="该标签下暂无笔记" @refresh="handleGetData" />
		</view>
