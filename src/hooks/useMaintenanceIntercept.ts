...
export const MAINTENANCE_PLUGIN_ID = 'uni-halo'
...
export function useMaintenanceIntercept() {
  ...
  function redirectToMaintenance(reason: MaintenanceInterceptReason) {
    console.log('跳转维护页', MAINTENANCE_PAGE_PATH, reason)
    uni.redirectTo({ url: `${MAINTENANCE_PAGE_PATH}?from=${reason}`, fail: (err) => {
      console.error('跳转维护页失败', err)
    } })
  }
