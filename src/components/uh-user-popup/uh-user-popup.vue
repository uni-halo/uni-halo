  ...
  >
    <view class="box-border h-full w-52 flex flex-col gap-y-6 px-3 pt-safe">
      ...
      <view v-if="hasLogin" class="w-full">
        <view class="uh-global-card-glass flex items-center gap-x-2 border rounded-xl p-3 shadow-none !bg-white/5">
          <image :src="userInfo.avatar" class="uh-global-card-glass uh-shadow-xs h-10 w-10 border rounded-full" />
          <view class="flex flex-col justify-center gap-y-1">
            <text class="text-2xs text-gray-900 font-semibold">{{ userInfo.nickname }}</text>
            <text class="rounded-md bg-secondary px-2 py-0.5 text-10px text-gray-500">{{ isAdmin ? '超级管理员' : '普通用户' }}</text>
          </view>
        </view>
      </view>
        ...
        <scroll-view scroll-y :show-scrollbar="false" class="mt-3 flex-1">
          <view class="box-border flex flex-col gap-y-3">
            <uh-permission v-for="entry in visibleAdminEntries" :key="entry.key" :permission="entry.permission">
              ...
              >
                <wd-icon :name="entry.icon" size="36rpx" custom-class="text-gray-900 dark:text-gray-100" />
