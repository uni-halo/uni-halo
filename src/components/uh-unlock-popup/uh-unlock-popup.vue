  ...
  >
    ...
    <view class="box-border w-full flex flex-col gap-y-3 p-3">
      ...
      <view class="w-full flex items-center justify-between">
        <view class="flex items-center gap-x-1 text-love font-bold">
          <wd-icon name="lock" size="42rpx" /> {{ title }}
        </view>
        <view
          v-if="closeable"
      ...
      <input
        v-model="password" :password="true" :placeholder="placeholder"
        class="uh-global-card-glass uh-shadow-xs box-border h-9 border rounded-xl px-3 text-xs"
      >

      <view v-if="captchaEnabled && captchaSrc" class="flex items-center justify-center gap-4">
        <input
          v-model="captchaCode" placeholder="验证码"
          class="uh-global-card-glass uh-shadow-xs box-border h-9 flex-1 border rounded-xl px-3 text-xs"
        >
        <image
          :src="captchaSrc" class="h-9 w-24 shrink-0 rounded-xl" mode="widthFix"
