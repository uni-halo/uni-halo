...
<template>
	<view
		...
		<!-- 上一页（6 个月） -->
		<view class="box-border shrink-0 flex items-center justify-center p-1 text-gray-500" @click="handlePrevPage">
			<wd-icon name="left" size="32rpx" />
		</view>

		<!-- 月份主体 -->
		<view class="box-border flex flex-1 flex-row items-center justify-between px-1">
			<view v-for="item in months" :key="item.month"
				...
				<text class="text-sm text-gray-900 font-semibold">{{ item.text }}</text>
				<text v-if="props.showYear" class="text-10px text-gray-500" :class="item.isSelected ? 'text-gray-900' :''">{{ item.month.split('-')[0] }}</text>
			</view>
		</view>

		<!-- 下一页（6 个月） -->
		<view class="shrink-0 flex items-center justify-center p-1 text-gray-500" @click="handleNextPage">
