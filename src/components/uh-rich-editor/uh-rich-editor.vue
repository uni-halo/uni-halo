 ...
 */
	...
	const toolbarItems : Array<ToolbarItem> = [
		{ label: 'bold', labelType: 'icon', title: '加粗', classNames: '', action: () => handleFormat('bold'), bold: true },
		{ label: 'italic', labelType: 'icon', title: '斜体', classNames: '', action: () => handleFormat('italic'), italic: true },
		{ label: 'underline', labelType: 'icon', title: '下划线', action: () => handleFormat('underline'), underline: true },
		{ label: 'strikethrough', labelType: 'icon', title: '删除线', action: () => handleFormat('strike'), strike: true },
		{ label: 'h2', labelType: 'icon', title: '标题', action: () => handleFormat('header', 2) },
		{ label: 'unordered-list', labelType: 'icon', title: '无序列表', action: () => handleFormat('list', 'bullet') },
		{ label: 'ordered-list', labelType: 'icon', title: '有序列表', action: () => handleFormat('list', 'ordered') },
		{ label: 'oblique-line', labelType: 'icon', title: '分割线', action: handleInsertDivider },
		{ label: 'image', labelType: 'icon', title: '插入图片', action: handleChooseImage },
	]
...
<template>
	<view class="w-full">
		<view v-if="props.toolbar && !props.readOnly"
			class="flex flex-wrap items-center gap-x-1 gap-y-1 border-b border-black/5 p-1">
			<view v-for="item in toolbarItems" :key="item.title"
				class="h-7 flex items-center justify-center rounded-md px-1.5 text-3xs text-gray-700 active:bg-black/5"
				:class="[item.classNames]" @click.stop="item.action">
				<text v-if="item.labelType==='text'">{{ item.label }}</text>
				<wd-icon v-else-if="item.labelType==='icon'" :name="item.label" size="32rpx" />
			</view>
		</view>
		<editor :id="editorId" class="w-full text-sm text-gray-900 p-2" :class="props.customClass"
