...
function checkIsVoted() {
  isVoted.value = voteCacheUtil.has(props.voteId)
}
...
<template>
  <view class="uh-global-card-glass box-border w-full border border-primary rounded-lg p-3 shadow-none">
    <view
      v-if="loading === 'error'" class="vote-error py-6 text-center text-xs text-gray-400"
      @click="handleGetData"
    >
      {{ loadingText }}
    </view>
    ...
    <template v-else-if="voteData">
      <view class="vote-card-head">
        <view class="flex items-center justify-between">
          ...
          <view
            class="flex shrink-0 items-center justify-center text-xs text-gray-400"
            @click="handleToVoteDetail"
          >
            投票详情 <wd-icon name="right" class="mt-0.5" />
          </view>
        </view>
      ...
      <view class="vote-card-body mt-2">
        ...
        <view v-if="voteData.spec?.type === 'single'" class="flex flex-col gap-2">
          <template v-if="showResult">
            <view
              v-for="(option, optionIndex) in voteData.spec?.options || []" :key="optionIndex"
              class="is-voted-item relative box-border min-h-9 overflow-hidden rounded-lg text-xs"
              :class="option.checked ? 'bg-primary text-gray-900 font-semibold' : 'bg-gray-100'"
              :style="{ '--percent': `${handleCalcPercent(option)}%` }"
            >
              <view class="relative z-2 box-border min-h-9 px-4 py-2.5">
                <view class="flex items-center justify-between">
        ...
        <view v-else-if="voteData.spec?.type === 'pk'" class="flex flex-col gap-2">
          ...
          <view class="pk-container box-border w-full flex">
            ...
            >
              <view
                class="option-item box-border w-full rounded-xl px-4 py-2.5"
          ...
          <template v-if="showResult">
            <view
              ...
              :style="{ '--percent': `${handleCalcPercent(option)}%` }"
            >
              <view class="relative z-2 box-border px-4 py-2.5">
                <view class="flex items-center justify-between">
                  <view class="flex-1 text-left">
          ...
          <template v-else>
            <view
              v-for="(option, optionIndex) in voteData.spec?.options || []" :key="optionIndex"
              class="uh-global-card-glass uh-shadow-xs box-border border rounded-xl bg-gray-100 px-4 py-2.5 text-xs"
              :class="option.checked ? 'bg-primary text-gray-900 font-semibold' : ''"
              @click="handleSelectSingleOption(option)"
            >
              选项{{ optionIndex + 1 }}：{{ option.title }}
