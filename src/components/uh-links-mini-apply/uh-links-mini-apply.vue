...
<template>
  <uh-glass-popup
    v-model="isShow" :z-index="100" position="bottom" :close-on-click-modal="false" custom-class="!border rounded-xl"
        ...
        <view class="w-full flex flex-col gap-y-3">
          <view class="flex items-center">
            ...
            <input
              ...
              class="uh-global-card-glass box-border h-9 flex-1 border rounded-xl px-4 text-3xs shadow-none"
          ...
          <view v-if="captchaSrc" class="captcha-box flex items-center">
            <text class="label w-[140rpx] shrink-0 text-3xs text-gray-600">验证码 *</text>
            <image
              :src="captchaSrc" class="captcha-img h-[76rpx] w-[200rpx] shrink-0 rounded-lg"
              mode="widthFix" @click="handleRefreshCaptcha"
            />
            <input
              v-model="captchaCode"
              class="input ml-3 box-border h-[72rpx] flex-1 rounded-xl bg-[#f5f5f5] px-4 text-3xs"
              placeholder="输入图中字符"
            >
          </view>
        </view>
