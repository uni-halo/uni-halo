...
<template>
  <view class="uh-global-card-glass uh-shadow-xs box-border w-full rounded-2xl p-4" @click="handleToDetail">
    ...
    <view class="flex items-center justify-between">
      <view class="flex flex-wrap items-center gap-1">
        <text
          ...
          class="rounded-md bg-gray-900 px-1.5 py-0.5 text-xs text-white"
        >
          {{ vote.spec._uh_type }}
        </text>
        <text
          v-if="vote.spec?._uh_state" class="rounded-md px-1.5 py-0.5 text-xs"
          :class="[vote.spec._uh_state.color, vote.spec._uh_state.bgColor]"
        >
          {{ vote.spec._uh_state.state }}
        </text>
        <text
          v-if="vote.spec?.isVoted"
          class="rounded-md bg-secondary px-1.5 py-0.5 text-xs text-gray-900"
        >
          已投票
        </text>
      </view>
    ...
    <!-- 标题 + 备注 -->
    <view class="vote-title mt-2 text-sm text-gray-900 font-bold">
      {{ vote.spec?.title }}
    </view>
