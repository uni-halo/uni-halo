	...
	<view class="uh-global-card-glass uh-shadow-xs box-border w-full rounded-2xl p-4" @click="handleToDetail">
		...
		<view class="flex items-center justify-between">
			<view class="flex flex-wrap items-center gap-1">
				<text v-if="vote.spec?._uh_type"
					class="rounded-md bg-secondary px-1.5 py-0.5 text-xs text-[#4d7c0f]">
					{{ vote.spec._uh_type }}
				</text>
				<text v-if="vote.spec?._uh_state" class="rounded-md px-1.5 py-0.5 text-xs"
					:class="[vote.spec._uh_state.color, vote.spec._uh_state.bgColor]">
					{{ vote.spec._uh_state.state }}
				</text>
				<text v-if="vote.spec?.isVoted"
					class="rounded-md bg-primary/15 px-1.5 py-0.5 text-xs text-primary">
					已投票
				</text>
			</view>
		...
		<!-- 标题 + 备注 -->
		<view class="vote-title mt-2 text-sm font-bold text-gray-900">
			{{ vote.spec?.title }}
		</view>
		...
		<view class="vote-card-foot mt-3 flex items-center justify-between border-t border-black/5 pt-2">
			<text v-if="vote.spec?.timeLimit === 'permanent'" class="text-xs text-gray-400">
				结束：永久有效
			</text>
			<text v-else-if="vote.spec?._uh_state?.state === '未开始'" class="text-xs text-gray-400">
				开始：{{ formatTime(vote.spec?.startDate) }}
			</text>
			<text v-else class="text-xs text-gray-400">
				结束：{{ formatTime(vote.spec?.endDate) }}
			</text>
			<text class="text-xs text-gray-400">{{ vote.stats?.voteCount || 0 }} 人已参与</text>
		</view>
	</view>
</template>