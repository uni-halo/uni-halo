    ...
    <view class="uh-global-card-glass uh-shadow-xs box-border rounded-xl p-3">
      ...
      <uh-section-title>
        ...
        <template #right>
          <view class="flex items-center gap-3">
            <text
              v-if="props.allowComment" class="text-xs text-gray-500 font-normal"
              @click="emit('on-comment-entry', props.post.metadata.name)"
            >
              写一个
            </text>
      ...
      <view class="mt-2">
        <view v-if="loadingStatus !== DataLoadingStatusEnum.Success">
          <uh-data-loading
            min-height="32vh" :loading-status="loadingStatus" size="small"
        ...
        <block v-else>
          ...
          <template v-for="comment in dataList" :key="comment.metadata.name">
            <uh-comment-item
              :use-content-bg="false" :is-child="false" :comment="comment"
            ...
            <view v-if="getReplyCount(comment) > 0">
              <!-- 未展开:仅显示回复数 -->
              <view
                v-if="!expanded.has(comment.metadata.name)"
                class="ml-10 mt-2 inline-flex items-center rounded-full bg-gray-100 px-3 py-0.5 text-xs text-gray-500"
              ...
              <block v-else>
                <view
                  v-if="getRepliesState(comment)?.status === 'error'"
                  class="ml-10 mt-2 text-xs text-red-400" @click="loadReplies(comment)"
                >
                  回复加载失败,点击重试
                </view>
                <view
                  v-else-if="getRepliesState(comment)?.status === 'loading' && getRepliesState(comment)?.list.length === 0"
                  class="ml-10 mt-2 text-xs text-gray-400"
                >
                  回复加载中...
                </view>
                <template v-else>
                  <uh-comment-item
                    v-for="childComment in getRepliesState(comment)?.list"
                  ...
                  <!-- 加载更多(响应 hasNext) -->
                  <view
                    v-if="getRepliesState(comment)?.hasNext"
                ...
                <!-- 收起 -->
                <view class="ml-10 mt-2 text-xs text-gray-400" @click="toggleReplies(comment)">
                  收起回复
                </view>
              </block>
            </view>
