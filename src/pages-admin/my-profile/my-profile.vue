	...
	function chooseAvatar() {
		...
		uni.chooseMedia({
			...
			success: (res) => {
				...
				if (file?.tempFilePath) { onPicked(file.tempFilePath) }
			},
		})
		// #endif
		// #ifndef MP-WEIXIN
	...
	<view class="box-border min-h-screen bg-page">
		...
		<view class="box-border flex flex-col gap-y-5 px-4 py-3">
			...
			<view class="uh-global-card-glass uh-shadow-xs flex flex-col items-center gap-y-2 rounded-2xl py-6"
				@click="chooseAvatar">
				<view class="relative h-22 w-22">
					<image :src="checkAvatarUrl(userInfo.avatar)"
						class="uh-global-card-glass uh-shadow-xs w-full h-full rounded-full" mode="aspectFill" />
					<view
					...
					<view class="flex items-center gap-x-3 px-4 py-3"
						...
						<view class="flex flex-1 items-center justify-end gap-x-2">
							<template v-if="editingNickname">
								<wd-input v-model="nicknameDraft" no-border
									custom-class="uh-profile-input uh-global-card-glass uh-shadow-xs border !rounded-lg !h-auto !py-1 !bg-gray-100 !text-xs"
									focus placeholder="请输入昵称" :disabled="nicknameSaving" :maxlength="20"
									@confirm="saveNickname" />
								<uh-button
									custom-class="shrink-0 uh-global-card-glass border !bg-gray-100 !px-2 !text-xs text-gray-gray-600"
									@click="cancelEditNickname">取消</uh-button>
								<uh-button custom-class="uh-global-card-glass shrink-0 !px-2 !text-xs text-gray-900"
									:class="nicknameSaving ? 'opacity-60' : ''" @click="saveNickname">
									{{ nicknameSaving ? '保存中' : '保存' }}
								</uh-button>
							</template>
							<template v-else>
								<text class="truncate text-2xs text-gray-500"
									@click="!editingNickname && startEditNickname()">{{ userInfo.nickname || '-' }}</text>
								<wd-icon name="edit" size="28rpx" custom-class="text-gray-500"
		...
		<uh-glass-popup v-model="passwordSheet" :hide-when-close="false" position="bottom" :z-index="100"
			...
			<view class="w-full box-border flex flex-col gap-y-3 p-3">
				...
				<view class="flex flex-col gap-y-3">
					...
					<wd-input v-model="confirmPassword" custom-class="uh-profile-input" show-password prefix-icon="lock"
